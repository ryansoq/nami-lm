...
class GPTMini(Module):
    ...
    def generate(self, token_ids, max_new=50, temperature=0.1,
        ...
        if rep_penalty is None:
            # HYP77 KEEP: rep penalty broke the greedy-argmax strict-39 ceiling
            # (33→41). HYP82: on the persona-pure HYP81 model, the optimum is
            # 1.6 → strict 42 / any-hit 50 (both best). 1.2-1.4→41, 1.8/2.0→41,
...
def train(epochs: int = 200, lr: float = 0.002,
    ...
    BATCH_SIZE = 8  # HYP67 REVERT — batch 4 → strict 36 = -3 from HYP61's 39. Restore 8. Phase 10 ceiling locked at strict 39 (all 5 scale directions tried + reverted). Phase 11 will expand corpus instead of tweaking optimizer.
